from typing import Concatenate, Any, Iterable, Callable
...
class Towell(StructuringFuzzifier):
    ...
    def __init__(self, predictor_input: Tensor, feature_mapping: dict[str, int], omega: float = 4):
        ...
        self.__rhs: dict[str, list[Tensor]] = {}
        self.__rhs_predicates: dict[str, tuple[dict[str, int], list[Callable]]] = {}
    ...
    def _visit_definition_clause(self, node: DefinitionClause, rhs: Clause, local_mapping: dict[str, int] = None):
        ...
        if predication_name is not None:
            ...
            if predication_name not in self.classes.keys():
                ...
                self.__rhs[predication_name] = [net]
            else:
                ...
                self.__rhs[predication_name] = incomplete_function.append(net)
                w = len(self.__rhs[predication_name]) * [self.omega]
                self.classes[predication_name] = self._operation.get('∨')(w)(self.__rhs[predication_name])
        else:
            ...
            if definition_name not in self._predicates.keys():
                ...
                self.__rhs_predicates[definition_name] = (local_mapping, [net])
            else:
                # Substitute variables that are not matching features with mapping functions
                nets = self.__rhs_predicates[definition_name][1].append(net)
                self.__rhs_predicates[definition_name] = (local_mapping, nets)
                self._predicates[definition_name] = (local_mapping,
                                                     lambda m: self._operation.get('∨')(w)
                                                     (self.__rhs_predicates[definition_name][1]))
    ...
    def _visit_expression(self, node: Expression, local_mapping: dict[str, int] = None) -> tuple[any, float]:
        ...
        if len(node.nary) < 1:
            ...
            previous_layer, w = layer_and_w[::2], layer_and_w[1::2]
    ...
    def _visit_variable(self, node: Variable, local_mapping: dict[str, int] = None) -> tuple[any, float]:
        ...
        if node.name in self.feature_mapping.keys():
            return Lambda(lambda x: gather(x, [self.feature_mapping[node.name]], axis=1))(self.predictor_input), self.omega
