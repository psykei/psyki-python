...
class NetworkStructurer(Injector):
    ...
    def __init__(self, predictor: Model, feature_mapping: dict[str, int], fuzzifier: str, layer: int = 0):
        ...
        self._base: Model = model_deep_copy(predictor)
        ...
        self._fuzzifier = Fuzzifier.get(fuzzifier)([self._base.input, feature_mapping])
    ...
    def inject(self, rules: List[Formula]) -> Model:
        ...
        predictor_input: Tensor = self._base.input
        modules = self._fuzzifier.visit(rules_copy)
        if self._layer == 0:
            # Injection!
            x = Concatenate(axis=1)([predictor_input] + modules)
            self._base.layers[1].build(x.shape)
            for layer in self._base.layers[1:]:
                x = layer(x)
        else:
            x = self._base.layers[1](predictor_input)
            for layer in self._base.layers[2:self._layer + 1]:
                x = layer(x)
            # Injection!
            x = Concatenate(axis=1)([x] + modules)
            self._base.layers[self._layer + 1].build(x.shape)
            x = self._base.layers[self._layer + 1](x)
            for layer in self._base.layers[self._layer + 2:]:
