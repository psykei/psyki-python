...
class NetBuilder(StructuringFuzzifier):
        ...
        if output_value is not None:
            ...
            if output_value is not None and not output_value[0].isupper():
                if output_value not in self.classes.keys():
                    self.classes[output_value] = self._visit(
                        rhs, local_mapping, substitutions
                    )
                    self.class_call[output_value] = self._visit(
                        rhs, local_mapping, substitutions
                    )
                else:
                    incomplete_rule: Tensor = self.class_call[output_value]
                    self.classes[output_value] = maximum(
                        incomplete_rule, self._visit(rhs, local_mapping, substitutions)
                    )
                    self.class_call[output_value] = maximum(
                        incomplete_rule, self._visit(rhs, local_mapping, substitutions)
                    )
        else:
            ...
            if predicate_name not in self.predicate_call_mapping.keys():
                ...
                self.predicate_call_mapping[predicate_name] = (
                    lambda m: lambda s: self._visit(rhs, m, s),
                    local_args,
                )
            else:
                ...
                self.predicate_call_mapping[predicate_name] = (
                    lambda m: lambda s: Maximum()(
                        [incomplete_function(m)(s), self._visit(rhs, m, s)]
                    ),
                    local_args,
                )
        ...
        for k, v in subs.items():
            ...
            def subs(v, idx):
                return gather(
                    transpose(
                        squeeze(
                            stack([self._visit(w, loc_copy, sub_copy) for w in v]),
                            axis=2,
                        )
                    ),
                    idx,
    ...
    def _visit_expression(self, node: Expression, local_mapping, substitutions):
        def concat(layers):
            new_layers = [
                l(self.predictor_input) if isinstance(l, Callable) else l
                for l in layers
            ]
        ...
        if node.is_optimized and node.op.is_optimizable:
            ...
            layer = [
                self._visit(node.lhs, local_mapping, substitutions),
                self._visit(node.rhs, local_mapping, substitutions),
            ]
        cases = [
